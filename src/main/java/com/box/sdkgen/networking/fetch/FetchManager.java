...
public class FetchManager {
    ...
    public static FetchResponse fetch(FetchOptions options) {
        ...
        while (true) {
            ...
            try {
                ...
                boolean acceptedWithRetryAfter = response.code() == 202 && response.header("Retry-After") != null;
                ...
                if (response.code() >= 400 && response.code() < 500 && response.code() != 429 && !authenticationNeeded) {
            ...
            try {
                ...
                int secondsToSleep = getRetryAfterTimeInSeconds(attemptNumber, response != null ? response.header("Retry-After"): null);
    ...
    private static RequestBody prepareRequestBody(FetchOptions options) {
        ...
        switch (contentType) {
            ...
            case "application/json-patch+json":
                return options.getData() != null ? RequestBody.create(sdToJson(options.getData()), MediaType.parse(contentType)) : null;
            case "application/x-www-form-urlencoded":
                return options.getData() != null ? RequestBody.create(sdToUrlParams(options.getData()), MediaType.parse(contentType)) : null;
    ...
    private static Response executeOnClient(OkHttpClient httpClient, Request request) throws IOException {
        return createNewCall(httpClient, request).execute();

    }

    private static void throwOnUnsuccessfulResponse(Request request, Response response, Exception exceptionThrown) {
        if(response == null){
