...
public class ZipDownloadsManager {
  ...
  public ZipDownload createZipDownload(ZipDownloadRequest requestBody, CreateZipDownloadHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/zip_downloads")).method("POST").headers(headersMap).data(JsonManager.serialize(requestBody)).contentType("application/json").responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
  ...
  public ByteStream getZipDownloadContent(String downloadUrl, GetZipDownloadContentHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(downloadUrl).method("GET").headers(headersMap).responseFormat("binary").auth(this.auth).networkSession(this.networkSession).build());
  ...
  public ZipDownloadStatus getZipDownloadStatus(String statusUrl, GetZipDownloadStatusHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(statusUrl).method("GET").headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
  ...
  public ByteStream downloadZip(ZipDownloadRequest requestBody, DownloadZipHeaders headers) {
    ZipDownload zipDownloadSession = this.createZipDownload(new ZipDownloadRequest.ZipDownloadRequestBuilder(requestBody.getItems()).downloadFileName(requestBody.getDownloadFileName()).build(), new CreateZipDownloadHeaders.CreateZipDownloadHeadersBuilder().extraHeaders(headers.getExtraHeaders()).build());
    return this.getZipDownloadContent(zipDownloadSession.getDownloadUrl(), new GetZipDownloadContentHeaders.GetZipDownloadContentHeadersBuilder().extraHeaders(headers.getExtraHeaders()).build());
  }
