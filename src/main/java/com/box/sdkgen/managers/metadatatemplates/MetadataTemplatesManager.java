...
public class MetadataTemplatesManager {
  ...
  public MetadataTemplates getMetadataTemplatesByInstanceId(GetMetadataTemplatesByInstanceIdQueryParams queryParams) {
    return getMetadataTemplatesByInstanceId(queryParams, new GetMetadataTemplatesByInstanceIdHeaders());
  }
  ...
  public MetadataTemplates getMetadataTemplatesByInstanceId(GetMetadataTemplatesByInstanceIdQueryParams queryParams, GetMetadataTemplatesByInstanceIdHeaders headers) {
    Map<String, String> queryParamsMap = prepareParams(mapOf(entryOf("metadata_instance_id", convertToString(queryParams.getMetadataInstanceId()))));
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_templates")).method("GET").params(queryParamsMap).headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
  ...
  public MetadataTemplate getMetadataTemplate(GetMetadataTemplateScope scope, String templateKey, GetMetadataTemplateHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_templates/", convertToString(scope), "/", convertToString(templateKey), "/schema")).method("GET").headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
  ...
  public MetadataTemplate updateMetadataTemplate(UpdateMetadataTemplateScope scope, String templateKey, List<UpdateMetadataTemplateRequestBody> requestBody) {
    return updateMetadataTemplate(scope, templateKey, requestBody, new UpdateMetadataTemplateHeaders());
  }
  ...
  public MetadataTemplate updateMetadataTemplate(UpdateMetadataTemplateScope scope, String templateKey, List<UpdateMetadataTemplateRequestBody> requestBody, UpdateMetadataTemplateHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_templates/", convertToString(scope), "/", convertToString(templateKey), "/schema")).method("PUT").headers(headersMap).data(JsonManager.serialize(requestBody)).contentType("application/json-patch+json").responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
    return JsonManager.deserialize(response.getData(), MetadataTemplate.class);
  }
  ...
  public void deleteMetadataTemplate(DeleteMetadataTemplateScope scope, String templateKey, DeleteMetadataTemplateHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_templates/", convertToString(scope), "/", convertToString(templateKey), "/schema")).method("DELETE").headers(headersMap).responseFormat(null).auth(this.auth).networkSession(this.networkSession).build());
  }
  ...
  public MetadataTemplate getMetadataTemplateById(String templateId, GetMetadataTemplateByIdHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_templates/", convertToString(templateId))).method("GET").headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
    return JsonManager.deserialize(response.getData(), MetadataTemplate.class);
  }
  ...
  public MetadataTemplates getGlobalMetadataTemplates(GetGlobalMetadataTemplatesQueryParams queryParams, GetGlobalMetadataTemplatesHeaders headers) {
    ...
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_templates/global")).method("GET").params(queryParamsMap).headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
    return JsonManager.deserialize(response.getData(), MetadataTemplates.class);
  }
  ...
  public MetadataTemplates getEnterpriseMetadataTemplates(GetEnterpriseMetadataTemplatesQueryParams queryParams, GetEnterpriseMetadataTemplatesHeaders headers) {
    Map<String, String> queryParamsMap = prepareParams(mapOf(entryOf("marker", convertToString(queryParams.getMarker())), entryOf("limit", convertToString(queryParams.getLimit()))));
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_templates/enterprise")).method("GET").params(queryParamsMap).headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
    return JsonManager.deserialize(response.getData(), MetadataTemplates.class);
  }
  ...
  public MetadataTemplate createMetadataTemplate(CreateMetadataTemplateRequestBody requestBody, CreateMetadataTemplateHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_templates/schema")).method("POST").headers(headersMap).data(JsonManager.serialize(requestBody)).contentType("application/json").responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
    return JsonManager.deserialize(response.getData(), MetadataTemplate.class);
  }
