...
public class MetadataCascadePoliciesManager {
  ...
  public MetadataCascadePolicies getMetadataCascadePolicies(GetMetadataCascadePoliciesQueryParams queryParams, GetMetadataCascadePoliciesHeaders headers) {
    Map<String, String> queryParamsMap = prepareParams(mapOf(entryOf("folder_id", convertToString(queryParams.getFolderId())), entryOf("owner_enterprise_id", convertToString(queryParams.getOwnerEnterpriseId())), entryOf("marker", convertToString(queryParams.getMarker())), entryOf("offset", convertToString(queryParams.getOffset()))));
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_cascade_policies")).method("GET").params(queryParamsMap).headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
  ...
  public MetadataCascadePolicy createMetadataCascadePolicy(CreateMetadataCascadePolicyRequestBody requestBody, CreateMetadataCascadePolicyHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_cascade_policies")).method("POST").headers(headersMap).data(JsonManager.serialize(requestBody)).contentType("application/json").responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
  ...
  public MetadataCascadePolicy getMetadataCascadePolicyById(String metadataCascadePolicyId) {
    return getMetadataCascadePolicyById(metadataCascadePolicyId, new GetMetadataCascadePolicyByIdHeaders());
  }
  ...
  public MetadataCascadePolicy getMetadataCascadePolicyById(String metadataCascadePolicyId, GetMetadataCascadePolicyByIdHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_cascade_policies/", convertToString(metadataCascadePolicyId))).method("GET").headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
    return JsonManager.deserialize(response.getData(), MetadataCascadePolicy.class);
  }

  public void deleteMetadataCascadePolicyById(String metadataCascadePolicyId) {
    deleteMetadataCascadePolicyById(metadataCascadePolicyId, new DeleteMetadataCascadePolicyByIdHeaders());
  }
  ...
  public void deleteMetadataCascadePolicyById(String metadataCascadePolicyId, DeleteMetadataCascadePolicyByIdHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_cascade_policies/", convertToString(metadataCascadePolicyId))).method("DELETE").headers(headersMap).responseFormat(null).auth(this.auth).networkSession(this.networkSession).build());
  }

  public void applyMetadataCascadePolicy(String metadataCascadePolicyId, ApplyMetadataCascadePolicyRequestBody requestBody) {
    applyMetadataCascadePolicy(metadataCascadePolicyId, requestBody, new ApplyMetadataCascadePolicyHeaders());
  }
  ...
  public void applyMetadataCascadePolicy(String metadataCascadePolicyId, ApplyMetadataCascadePolicyRequestBody requestBody, ApplyMetadataCascadePolicyHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/metadata_cascade_policies/", convertToString(metadataCascadePolicyId), "/apply")).method("POST").headers(headersMap).data(JsonManager.serialize(requestBody)).contentType("application/json").responseFormat(null).auth(this.auth).networkSession(this.networkSession).build());
  }
