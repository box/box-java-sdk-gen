...
public class LegalHoldPoliciesManager {
  ...
  public LegalHoldPolicies getLegalHoldPolicies(GetLegalHoldPoliciesQueryParams queryParams, GetLegalHoldPoliciesHeaders headers) {
    Map<String, String> queryParamsMap = prepareParams(mapOf(entryOf("policy_name", convertToString(queryParams.getPolicyName())), entryOf("fields", convertToString(queryParams.getFields())), entryOf("marker", convertToString(queryParams.getMarker())), entryOf("limit", convertToString(queryParams.getLimit()))));
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/legal_hold_policies")).method("GET").params(queryParamsMap).headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
  ...
  public LegalHoldPolicy getLegalHoldPolicyById(String legalHoldPolicyId, GetLegalHoldPolicyByIdHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/legal_hold_policies/", convertToString(legalHoldPolicyId))).method("GET").headers(headersMap).responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
    return JsonManager.deserialize(response.getData(), LegalHoldPolicy.class);
  }

  public LegalHoldPolicy updateLegalHoldPolicyById(String legalHoldPolicyId) {
    return updateLegalHoldPolicyById(legalHoldPolicyId, new UpdateLegalHoldPolicyByIdRequestBody(), new UpdateLegalHoldPolicyByIdHeaders());
  }
  ...
  public LegalHoldPolicy updateLegalHoldPolicyById(String legalHoldPolicyId, UpdateLegalHoldPolicyByIdRequestBody requestBody) {
    return updateLegalHoldPolicyById(legalHoldPolicyId, requestBody, new UpdateLegalHoldPolicyByIdHeaders());
  }

  public LegalHoldPolicy updateLegalHoldPolicyById(String legalHoldPolicyId, UpdateLegalHoldPolicyByIdHeaders headers) {
    return updateLegalHoldPolicyById(legalHoldPolicyId, new UpdateLegalHoldPolicyByIdRequestBody(), headers);
  }
  ...
  public LegalHoldPolicy updateLegalHoldPolicyById(String legalHoldPolicyId, UpdateLegalHoldPolicyByIdRequestBody requestBody, UpdateLegalHoldPolicyByIdHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/legal_hold_policies/", convertToString(legalHoldPolicyId))).method("PUT").headers(headersMap).data(JsonManager.serialize(requestBody)).contentType("application/json").responseFormat("json").auth(this.auth).networkSession(this.networkSession).build());
    return JsonManager.deserialize(response.getData(), LegalHoldPolicy.class);
  }
  ...
  public void deleteLegalHoldPolicyById(String legalHoldPolicyId, DeleteLegalHoldPolicyByIdHeaders headers) {
    Map<String, String> headersMap = prepareParams(mergeMaps(mapOf(), headers.getExtraHeaders()));
    FetchResponse response = fetch(new FetchOptions.FetchOptionsBuilder(String.join("", this.networkSession.getBaseUrls().getBaseUrl(), "/2.0/legal_hold_policies/", convertToString(legalHoldPolicyId))).method("DELETE").headers(headersMap).responseFormat(null).auth(this.auth).networkSession(this.networkSession).build());
  }
