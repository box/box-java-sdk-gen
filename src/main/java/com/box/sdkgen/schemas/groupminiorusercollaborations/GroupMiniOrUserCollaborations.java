...
@JsonDeserialize(using = GroupMiniOrUserCollaborations.GroupMiniOrUserCollaborationsDeserializer.class)
...
public class GroupMiniOrUserCollaborations extends OneOfTwo<GroupMini, UserCollaborations> {
  ...
  static class GroupMiniOrUserCollaborationsDeserializer extends JsonDeserializer<GroupMiniOrUserCollaborations> {
    ...
    @Override
    public GroupMiniOrUserCollaborations deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
      JsonNode node = JsonManager.jsonToSerializedData(jp);
      JsonNode discriminant0 = node.get("type");
      if(!(discriminant0 == null)){
        switch (discriminant0.asText()) {
          case "group":
            return new GroupMiniOrUserCollaborations(JsonManager.deserialize(node, GroupMini.class));
          case "user":
            return new GroupMiniOrUserCollaborations(JsonManager.deserialize(node, UserCollaborations.class));
        }
      }
