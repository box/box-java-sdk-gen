...
@JsonDeserialize(using = FileBaseOrFolderBaseOrWebLinkBase.FileBaseOrFolderBaseOrWebLinkBaseDeserializer.class)
@JsonSerialize(using = OneOfThree.OneOfThreeSerializer.class)
public class FileBaseOrFolderBaseOrWebLinkBase extends OneOfThree<FileBase, FolderBase, WebLinkBase> {
  ...
  static class FileBaseOrFolderBaseOrWebLinkBaseDeserializer extends JsonDeserializer<FileBaseOrFolderBaseOrWebLinkBase> {
    ...
    @Override
    public FileBaseOrFolderBaseOrWebLinkBase deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
      JsonNode node = JsonManager.jsonToSerializedData(jp);
      JsonNode discriminant0 = node.get("type");
      if(!(discriminant0 == null)){
        switch (discriminant0.asText()) {
          case "file":
            return new FileBaseOrFolderBaseOrWebLinkBase(JsonManager.deserialize(node, FileBase.class));
          case "folder":
            return new FileBaseOrFolderBaseOrWebLinkBase(JsonManager.deserialize(node, FolderBase.class));
          case "web_link":
            return new FileBaseOrFolderBaseOrWebLinkBase(JsonManager.deserialize(node, WebLinkBase.class));
        }
      }
