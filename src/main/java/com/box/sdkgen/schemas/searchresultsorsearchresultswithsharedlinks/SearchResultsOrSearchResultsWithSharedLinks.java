...
@JsonDeserialize(using = SearchResultsOrSearchResultsWithSharedLinks.SearchResultsOrSearchResultsWithSharedLinksDeserializer.class)
...
public class SearchResultsOrSearchResultsWithSharedLinks extends OneOfTwo<SearchResults, SearchResultsWithSharedLinks> {
  ...
  static class SearchResultsOrSearchResultsWithSharedLinksDeserializer extends JsonDeserializer<SearchResultsOrSearchResultsWithSharedLinks> {
    ...
    @Override
    public SearchResultsOrSearchResultsWithSharedLinks deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
      JsonNode node = JsonManager.jsonToSerializedData(jp);
      JsonNode discriminant0 = node.get("type");
      if(!(discriminant0 == null)){
        switch (discriminant0.asText()) {
          case "search_results_items":
            return new SearchResultsOrSearchResultsWithSharedLinks(JsonManager.deserialize(node, SearchResults.class));
          case "search_results_with_shared_links":
            return new SearchResultsOrSearchResultsWithSharedLinks(JsonManager.deserialize(node, SearchResultsWithSharedLinks.class));
        }
      }
      throw new JsonMappingException(jp, "Unable to deserialize SearchResultsOrSearchResultsWithSharedLinks");
    }
