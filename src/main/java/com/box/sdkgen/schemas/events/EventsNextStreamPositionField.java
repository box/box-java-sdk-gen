...
@JsonDeserialize(using = EventsNextStreamPositionField.EventsNextStreamPositionFieldDeserializer.class)
...
public class EventsNextStreamPositionField extends OneOfTwo<String, Double> {
  ...
  static class EventsNextStreamPositionFieldDeserializer extends JsonDeserializer<EventsNextStreamPositionField> {
    ...
    @Override
    public EventsNextStreamPositionField deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
      JsonNode node = JsonManager.jsonToSerializedData(jp);
      try {
        return new EventsNextStreamPositionField(OneOfTwo.OBJECT_MAPPER.convertValue(node, Double.class));
      } catch (Exception ignored) {
      }
      try {
        return new EventsNextStreamPositionField(OneOfTwo.OBJECT_MAPPER.convertValue(node, String.class));
      } catch (Exception ignored) {
      }
